...
void ds_print_reg(ds_Register reg) {
   ...
   //  char binary[reg.nq + 1]; // +1 for the null terminator  // Previous definition of binary (as a variable length array)

    char* binary = malloc(reg.nq + 1); // using malloc to avoid variable length array
    if (binary == NULL){
      printf("Failed to allocate memory for array 'binary' in ds_print_reg");
    }


    for (i = 0; i < reg.nc; i++) {
        if (reg.state[i].x != 0 || reg.state[i].y != 0) {
            // Reset the binary string for each state
            memset(binary, '0', reg.nq); // Initialize all to '0'
            binary[reg.nq] = '\0'; // Set null terminator

            // Convert index i to binary, stored in reverse
            int temp = i;
            j = reg.nq - 1;
            while (temp > 0 && j >= 0) {
                binary[j--] = (temp % 2) + '0'; // Convert bit to char '0' or '1'
                temp /= 2;
            }

            // Print the binary string and the corresponding complex number
            printf("%s: %g + %gi\n", binary, reg.state[i].x, reg.state[i].y);
        }
    }
    printf("\n");
    free(binary);
...
{ // 0 stab type means X-type so do Hadamard
    if (stab_type == 0) {
        ds_Hadamard(reg, auxindex, 0); // Hadamard on aux
        ds_lerr(reg, auxindex, 1);     // gate error
  // Otherwise do idling error if idlings turned on:      
    } else if (stab_type == 1 && idling_errors) {
        ds_lerr(reg, auxindex, 1);     // aux idling error
                    }   
}
...
{
    if (idling_errors && num_preceding_data_idling_errors !=0){
    for (int i = 0; i < num_preceding_data_idling_errors; i++){
        ds_lerr(reg, data_qubit_index, 1); // data idling error
    }
    }
}
...
{
    if (idling_errors && num_idling_errors !=0){
    for (int i = 0; i < num_idling_errors; i++){
        ds_lerr(reg, data_qubit_index, 1); // data idling error
    }
    }
}
...
void ds_idling_during_TI(ds_Register reg, int auxindex)
{// Apply the idling errors that would be on this auxiliary while waiting for transversal injection before the first sweep
        ds_lerr(reg, auxindex, 1); 
        ds_lerr(reg, auxindex, 1); 
}
...
{
    if (stab_type ==0) {
        ds_cnot(reg,auxindex,k,0); // X-type stabs have aux as control
    } else if (stab_type == 1) {
        ds_cnot(reg,k,auxindex,0);
    }
    ds_lerr(reg,auxindex,1); // gate error
    ds_lerr(reg, k, 1); // gate error

}

void ds_aux_idling(ds_Register reg,int auxindex, int aux_idling, int idling_errors)
{
   if (idling_errors && aux_idling != 0){
      for (int i = 0; i < aux_idling; i++){
         ds_lerr(reg,auxindex,1); // aux idling error
      }
   }   
}
...
int ds_get_qubit_step(ds_Register reg, int qubit_index) {
   if (qubit_index >= 0 && qubit_index < reg.nq) {
       return reg.steps[qubit_index];
   } else {
       fprintf(stderr, "Error: qubit_index out of bounds in ds_get_qubit_step\n");
...
void ds_set_qubit_step(ds_Register reg, int qubit_index, int step_value) {
   if (qubit_index >= 0 && qubit_index < reg.nq) {
       reg.steps[qubit_index] = step_value;
   } else {
       // Optionally, handle the error condition if the qubit_index is out of bounds
       fprintf(stderr, "Error: qubit_index out of bounds in ds_set_qubit_step\n");
   }
} // This will set the reg.steps (timesteps) that a qubit has gone through to the number specified. Normally if, for example, an auxiliary is used to measure a stabiliser and then a different stabiliser afterwards, its reg.steps will have added up and when it goes to act on the other qubits their reg.steps will be less, so faulty identity gates will be applied on them to simulate their idling. This function can be used to re-set the auxliary's reg.steps, simulating that you were actually running them all in parallel.

void ds_print_qubit_step(ds_Register reg, int qubit_index) {
   if (qubit_index >= 0 && qubit_index < reg.nq) {
       printf("Qubit %d step count: %d\n", qubit_index, reg.steps[qubit_index]);
   } else {
       fprintf(stderr, "Error: qubit_index out of bounds in ds_print_qubit_step\n");
   }
}

void ds_print_all_qubit_steps(ds_Register reg) {
   printf("Qubit steps:\n");
   for (int i = 0; i < reg.nq; i++) {
       printf("Qubit %d: %d steps\n", i, reg.steps[i]);
   }
}
