...
// (Normally 12 auxiliarys but I'm just using one and resetting it)
    ...
    if (output == NULL) {
        perror("Hi! Failed to open file -- this might be because you're running d3rabi_v4 directly whereas the path is designed so that it can be run by RUN_RABI.bash");
